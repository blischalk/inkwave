...
function blocksToContent(blocks) {
  ...
  var result = cleanRaw(blocks[0]);
  for (var i = 1; i < blocks.length; i++) {
    var prevType = typeof blocks[i - 1] === "object" ? blocks[i - 1].type : "paragraph";
    var curType  = typeof blocks[i]     === "object" ? blocks[i].type     : "paragraph";
    // Consecutive list items use a single newline (tight list, no blank lines between items).
    var sep = (prevType === "list" && curType === "list") ? "\n" : "\n\n";
...
function renderTabBar() {
  ...
  tabs.forEach(function (tab, index) {
    ...
    btn.addEventListener("dragend", function (e) {
      ...
      tabBarEl.querySelectorAll(".tab.drop-target").forEach(function (el) {
...
function startInlineEdit(blockEl, index, blocks, tab, clickEvent) {
  ...
  var explicitOffset =
    arguments.length > 5 && typeof arguments[5] === "number"
      ? arguments[5]
      : null;

  var isListItemInPlace =
    blockEl.tagName === "LI" &&
    blockEl.parentNode &&
  ...
  if (isListItemInPlace) {
    ...
    blockEl.addEventListener("keydown", function (e) {
      ...
      if (
        ...
        showTabContent(tab);
        setTimeout(function () {
          var prev = contentEl.querySelector(
      ...
      if (e.key === "Enter") {
        ...
        if (isLastBlock && isEmptyItem) {
          ...
          requestAnimationFrame(function () {
            setTimeout(function () {
              var nextEl = contentEl.querySelector(
        ...
        requestAnimationFrame(function () {
          showTabContent(tab, blocks);
          setTimeout(function () {
            var newLi = contentEl.querySelector(
              '.md-block[data-block-index="' + (index + 1) + '"]',
            );
            if (newLi) {
  ...
  var sourceOffset = renderedOffsetToSourceOffset(
    raw,
    renderedText,
    offsetInRendered,
    blockType,
  ...
  editable.addEventListener("keydown", function (e) {
    ...
    if (e.key === "Backspace" && index > 0 && getEditableText().trim() === "") {
      ...
      showTabContent(tab);
      setTimeout(function () {
        var prevBlockEl = contentEl.querySelector(
          '.md-block[data-block-index="' + (index - 1) + '"]',
        );
        if (prevBlockEl)
          startInlineEdit(
      ...
      if (blockEl.classList.contains("md-block-code")) {
        ...
        if (openFenceMatch && onlyFenceLine) {
          ...
          requestAnimationFrame(function () {
            ...
            setTimeout(function () {
              var codeBlockEl = contentEl.querySelector(
                '.md-block[data-block-index="' + index + '"]',
              );
        ...
        if (/```[ \t]*$/.test(lastLine) && atEndOfLastLine) {
          ...
          requestAnimationFrame(function () {
            showTabContent(tab, currentBlocks);
            setTimeout(function () {
              var newBlockEl = contentEl.querySelector(
                '.md-block[data-block-index="' + (index + 1) + '"]',
              );
...
function showTabContent(tab, preferredBlocks) {
  ...
  try {
    ...
    if (isEmptyFile) {
      var firstBlock = contentEl.querySelector(
        '.md-block[data-block-index="0"]',
      );
      if (firstBlock) {
        setTimeout(function () {
          startInlineEdit(firstBlock, 0, currentBlocks, tab, null);
        }, 0);
      }
    }
  } finally {
